 ...
 * anonymous browsing early. /tv, /live, /api/boards, /api/version and /sounds
 * (the wall display's celebration audio) are deliberately public
 * (data-minimal board numbers for the wall display); /api/ingest is public
 * but guarded by its own INGEST_SECRET bearer token.
 *
...
export const config = {
  matcher: [
    "/((?!sign-in|tv|live|api/boards|api/version|api/cron|api/ingest|sounds|_next/static|_next/image|favicon.ico|icon.svg|robots.txt).*)",
  ],
};
