...
/**
 * Full-screen DARK wall leaderboard for /live — black background, each rep's
 * box glowing in their gender colour (pink/blue). Shows the dashboard's three
 * boards (Today, This month with incentive-tier gaps, Next 3 months), polls
 * live, and runs the per-booking celebration.
 */

const EMOJI: Record<string, string> = { hit: "🎉", over: "🔥", wild: "🐐" };

/** Glowing outline for a rep's box, by gender. */
function glow(g: "f" | "m" | "x"): string {
  if (g === "f") return "ring-1 ring-pink-400/70 shadow-[0_0_24px_-2px_rgba(244,114,182,0.9)]";
  if (g === "m") return "ring-1 ring-sky-400/70 shadow-[0_0_24px_-2px_rgba(56,189,248,0.9)]";
  return "ring-1 ring-white/25 shadow-[0_0_16px_-3px_rgba(255,255,255,0.45)]";
}
...
export function LiveBoard({ initial }: { initial: BoardsDTO }) {
  ...
  return (
    <main className="ink-grain relative flex h-dvh flex-col gap-3 overflow-hidden bg-black p-4 text-white sm:p-5">
      ...
      <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-12">
        {/* TODAY — glowing leaderboard tiles */}
