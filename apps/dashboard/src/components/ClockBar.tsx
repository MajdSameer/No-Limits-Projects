...
export function ClockBar({ status, sinceISO, workedMs, actions }: ClockBarProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/90 shadow-[0_-4px_20px_rgba(15,25,47,0.06)] backdrop-blur">
        ...
        <p className="min-w-0 truncate text-sm">
          <span
            ...
            className={cx(
              ...
              status === "on" && "bg-accent-500",
              status === "break" && "bg-brand-400",
              status === "off" && "bg-slate-300",
              status === "done" && "bg-brand-600",
            )}
          />
          <span className="font-semibold text-brand-900">{STATUS_LABEL[status]}</span>
          {status !== "off" && (
          ...
          {error && (
            <span role="alert" className="ml-3 font-semibold text-brand-700">
              {error}
            </span>
          )}
        </p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {actions.map((kind, i) => (
            <button
              ...
              className={cx(
                "min-h-11 rounded-full px-5 text-sm font-semibold transition-all focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-600 motion-safe:hover:-translate-y-0.5",
                i === 0
